...
average_conversation_time_by_level, time_spent_on_questions, nps_by_device
...
def create_xlsx(debut,fin,company) : 
	...
	# avg_time_on_qst = time_spent_on_questions(company, debut, fin)
	# print avg_time_on_qst
	...
	# volumes_by_UA_df = pd.DataFrame(volumes_by_UA, columns=['UA (% of volume)','% of volume','PoppedUp', 'Started', 'Lvl2','Lvl3','Lvl4','Lvl5','Lvl6','Lvl7','Lvl8','Exported'])
	# volumes_by_UA_df.to_excel(writer,sheet_name='volumes by level by UA', index = False)

	# columnsList=['UA (% of volume)', 'Started/PoppedUp', 'Lvl2/Started']
	# columnsList += ['Lvl'+str(x+1)+'/Lvl'+str(x) for x in range(2,8)]
	# columnsList.append('Exported/Lvl8')
	...
	# test = candidats_unique_exported(company, debut, fin)
